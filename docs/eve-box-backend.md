...
- `create`/resume: creates a Box through the v1 API, forks a prepared source Box when `forkFromBoxId` is set, or reconnects to `metadata.boxId` from Eve's persisted session state.
...
## Reusing a prepared Box with `forkFromBoxId`

When many Eve sessions should start from the same installed filesystem, prepare one source Box outside Eve, stop it, and wait for `snapshotAvailable: true`. Then configure the backend to fork that source for every new session:
...
`forkFromBoxId` can also be a function of Eve's create input, for example to choose different prepared boxes per `sessionKey`. The adapter calls `POST /boxes/{boxId}/fork`, applies `env`/`noEnv`, renames the fork, sets its TTL, waits for readiness, and initializes Eve's `/workspace` mapping. Forking copies files from the snapshot only; restart long-running processes in Eve setup when needed.
...
Template prewarming replays your seed files and bootstrap code when each session's box is created. For true snapshot clones, use `forkFromBoxId` as shown above.
